...
class beamformer():
    ...
    def process(self, frame):
        ...
        self.bf_out = np.multiply(np.multiply(w_temp[:, 0], np.conjugate(frame[:, 0])) + np.matmul(w_temp[:, 1], np.conjugate(frame[:, 1])) + np.matmul(w_temp[:, 2], np.conjugate(frame[:, 2])) + np.matmul(w_temp[:, 3], np.conjugate(frame[:, 3])), 1/self.alpha)
